...
// 5 screens, auto-cycle, with floating annotations that point at live elements.
// Hover for tilt + nav dots.
...
const MobileMockup = ({ paused: pausedProp }) => {
  const [active, setActive] = useState(0);
  const [internalPause, setInternalPause] = useState(false);
  const [tilt, setTilt] = useState({ x: 0, y: 0 });
  const wrapRef = useRef(null);

  const paused = pausedProp || internalPause;

  // Auto-advance
  useEffect(() => {
    if (paused) return;
    const t = setTimeout(() => setActive((a) => (a + 1) % SCREENS.length), SCREENS[active].duration);
  ...
  return (
    <div
      ...
      onMouseOut={() => setInternalPause(false)}
      style={{
        position: 'relative',
        perspective: '1600px',
        padding: '40px 0',
    ...
    >
      {/* Floating annotations — orbital cards */}
      ...
      {/* Dot navigation — visible always but very quiet */}
      <div style={{
        marginTop: 24,
        display: 'flex',
      ...
      <div style={{
        ...
        <span style={{ color: 'var(--ink-4)' }}>{String(active + 1).padStart(2, '0')} / {String(SCREENS.length).padStart(2, '0')}</span>
  ...
  <div style={{
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontFamily: 'var(--sans)', fontSize: 13, fontWeight: 600, color: 'var(--ink)' }}>9:41</div>
        <div style={{ display: 'flex', gap: 5, alignItems: 'center', color: 'var(--ink)' }}>
          {/* Signal */}
          <svg width="16" height="11" viewBox="0 0 16 11"><g fill="currentColor">
          ...
          {/* WiFi */}
          <svg width="14" height="11" viewBox="0 0 14 11" stroke="currentColor" fill="none" strokeWidth="1.4" strokeLinecap="round">
          ...
          <svg width="24" height="11" viewBox="0 0 24 11">
            <rect x="0.5" y="0.5" width="20" height="10" rx="2.5" fill="none" stroke="currentColor" strokeWidth="0.8" opacity="0.5"/>
...
// ─── Screen Stack — crossfade between screens ────────────────────────────────
const ScreenStack = ({ active }) => (
...
const ScreenDashboard = ({ live }) => {
  ...
  return (
    <div style={mock.scrollPad}>
      <div style={mock.eyebrow}>FILE 01 · ESTATE</div>
      ...
      {/* Stat ledger */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', borderTop: '1px solid var(--rule)', borderBottom: '1px solid var(--rule)' }}>
        {[
          ['Appliances', '142'],
          ['Tasks', '07'],
          ['Members', '08'],
          ...
          <div key={i} style={{
            ...
            <div style={{ fontFamily: 'var(--mono)', fontSize: 8.5, textTransform: 'uppercase', letterSpacing: '0.14em', color: 'var(--ink-3)', marginTop: 2 }}>{k}</div>
          </div>
        ))}
      </div>

      {/* Upcoming */}
      <div style={{ marginTop: 14, fontFamily: 'var(--mono)', fontSize: 9.5, textTransform: 'uppercase', letterSpacing: '0.14em', color: 'var(--ink-3)' }}>UPCOMING</div>
      <div style={{ marginTop: 8 }}>
        {[
          { d: 'MAR 12', t: 'Replace furnace filter', m: 'DC', acc: 'gold' },
          { d: 'MAR 17', t: 'Annual HVAC service',    m: 'DC', acc: 'accent' },
          { d: 'MAR 21', t: 'Generator service',      m: 'DC', acc: 'accent' },
        ].map((r, i) => (
          <div key={i} style={{
            display: 'flex',
            alignItems: 'center',
            ...
            <span className="num" style={{ fontSize: 9.5, color: 'var(--ink-3)', width: 50 }}>{r.d}</span>
            <span style={{ fontSize: 11, color: 'var(--ink)', flex: 1 }}>{r.t}</span>
            <span className="dot" style={{ background: r.acc === 'accent' ? 'var(--accent)' : 'var(--gold)' }} />
      ...
      {/* Rooms grid hint */}
      <div style={{ marginTop: 16, fontFamily: 'var(--mono)', fontSize: 9.5, textTransform: 'uppercase', letterSpacing: '0.14em', color: 'var(--ink-3)' }}>ROOMS · 24</div>
...
// ────────────────────────────── Screen 2: Receipt ────────────────────────────
const ScreenReceipt = ({ live }) => {
  const [step, setStep] = useState(0); // 0:scanning 1:detected 2:fields fill 3:done
  useEffect(() => {
  ...
  return (
    <div style={mock.scrollPad}>
      <div style={mock.eyebrow}>NEW RECEIPT</div>
      ...
      <div style={{
        ...
        <div style={{ padding: 12, fontFamily: 'var(--mono)', fontSize: 8 }}>
          ...
          <div style={{ borderTop: '1px dashed #ccc', paddingTop: 6, marginTop: 4 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 7.5 }}>
              <span>Wolf 36" GR Range</span><span>10,899.00</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 7.5 }}>
              <span>Install kit</span><span>89.00</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 7.5 }}>
              <span>Warranty +2yr</span><span>432.00</span>
            </div>
          </div>
          <div style={{ borderTop: '1px solid #000', marginTop: 8, paddingTop: 4, display: 'flex', justifyContent: 'space-between', fontSize: 8, fontWeight: 600 }}>
        ...
        {step < 2 && (
          <div style={{
            ...
            <div style={{
              ...
              animation: live ? 'scanSweep 2.4s ease-in-out infinite' : 'none',
            }}/>
            <div style={{
              position: 'absolute',
              left: 0,
              ...
              animation: live ? 'scanLine 2.4s ease-in-out infinite' : 'none',
            }}/>
          </div>
        )}
        ...
        {step >= 1 && (
          <div style={{
            ...
            borderRadius: 3,
            animation: 'pulseBox 1s ease-out',
          }}/>
        )}
      </div>
      ...
      <div style={{
        ...
        <span className="dot" style={{ background: step === 3 ? 'var(--green)' : 'var(--accent)', animation: step < 3 ? 'pulse 1.2s ease-in-out infinite' : 'none' }}/>
        <span>{step === 0 ? 'Scanning…' : step === 1 ? 'Receipt detected' : step === 2 ? 'Reading fields…' : 'Saved to Kitchen'}</span>
      </div>

      {/* Field rows that fill in one by one */}
      <div style={{ marginTop: 10 }}>
        {fields.map((f, i) => (
          <div key={i} style={{
            ...
            <span style={{ fontFamily: 'var(--mono)', fontSize: 8.5, textTransform: 'uppercase', letterSpacing: '0.14em', color: 'var(--ink-3)' }}>{f.k}</span>
            <span className="num" style={{ fontSize: 11, color: 'var(--ink)' }}>{f.v}</span>
          </div>
        ))}
      </div>
    </div>
...
// ────────────────────────────── Screen 3: Calendar ───────────────────────────
const ScreenCalendar = ({ live }) => {
  const [highlight, setHighlight] = useState(12);
  useEffect(() => {
    if (!live) return;
    const days = [12, 17, 21, 28];
    let i = 0;
    const t = setInterval(() => { i = (i + 1) % days.length; setHighlight(days[i]); }, 900);
    return () => clearInterval(t);
  }, [live]);

  const dots = { 4: 1, 8: 1, 12: 3, 17: 1, 21: 2, 23: 2, 28: 2, 30: 1 };

  return (
    <div style={mock.scrollPad}>
      <div style={mock.eyebrow}>MARCH 2026</div>
      <div className="display" style={{ fontSize: 22, lineHeight: 1.05, marginTop: 4, marginBottom: 14 }}>
        <span className="display-italic">Calendar</span>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 0, fontFamily: 'var(--mono)', fontSize: 8, color: 'var(--ink-3)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                    ...
                    <div style={{
                      ...
                      {Array.from({ length: dotCount }, (_, j) => (
                        <span key={j} style={{
                          ...
                          background: dotCount === 3 ? 'var(--accent)' : dotCount === 2 ? 'var(--gold)' : 'var(--green)',
                        }}/>
                      ))}
                    </div>
                  )}
      ...
      <div style={{ marginTop: 12 }}>
        <div style={{ fontFamily: 'var(--mono)', fontSize: 8.5, textTransform: 'uppercase', letterSpacing: '0.14em', color: 'var(--ink-3)' }}>
          MAR {String(highlight).padStart(2,'0')} · 03 TASKS
        </div>
        <div style={{ marginTop: 6 }}>
          {[
            { t: '08:00', n: 'Replace furnace filter', d: 'gold' },
            { t: '11:00', n: 'Descale espresso boiler', d: 'green' },
            { t: '15:00', n: 'Inspect attic insulation', d: 'green' },
          ].map((r, i) => (
            <div key={i} style={{
              ...
              <span className="num" style={{ fontSize: 9, color: 'var(--ink-3)' }}>{r.t}</span>
...
// ────────────────────────────── Screen 4: Vault ──────────────────────────────
const ScreenVault = ({ live }) => {
  const [step, setStep] = useState(0); // 0 locked, 1 unlocking, 2 unlocked
  const [pin, setPin] = useState(0);
  useEffect(() => {
    if (!live) { setStep(0); setPin(0); return; }
    const ts = [];
  ...
  return (
    <div style={mock.scrollPad}>
      <div style={mock.eyebrow}>FILE 06 · VAULT</div>
      ...
      {step < 2 ? (
        ...
        <div style={{ textAlign: 'center', padding: '20px 0' }}>
          ...
          {/* Keypad hint */}
          <div style={{
            display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6, maxWidth: 200, margin: '0 auto',
          }}>
            {['1','2','3','4','5','6','7','8','9'].map(n => (
              <div key={n} style={{
                ...
                borderRadius: 4,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontFamily: 'var(--mono)', fontSize: 14, color: 'var(--ink-2)',
              }}>{n}</div>
            ))}
          </div>
        </div>
        ...
        <div className="ifade">
          <div style={{
            ...
            padding: '4px 8px', border: '1px solid var(--green-soft)',
            background: 'var(--green-soft)', color: 'var(--green)',
            borderRadius: 3, fontFamily: 'var(--mono)', fontSize: 8.5,
          ...
          {[
            ...
            <div key={i} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ fontFamily: 'var(--mono)', fontSize: 8, color: 'var(--ink-3)', letterSpacing: '0.12em' }}>{r.k}</div>
                  <div style={{ fontSize: 11, color: 'var(--ink)', marginTop: 2 }}>{r.n}</div>
                </div>
                <div className="num" style={{ fontSize: 11, color: 'var(--ink-2)', letterSpacing: '0.05em' }}>{r.v}</div>
              </div>
            </div>
          ))}
        </div>
...
// ────────────────────────────── Screen 5: Binder ─────────────────────────────
const ScreenBinder = ({ live }) => {
  const [progress, setProgress] = useState(0);
  useEffect(() => {
    if (!live) { setProgress(0); return; }
    let p = 0;
    const t = setInterval(() => {
      p = Math.min(p + 4, 100);
      setProgress(p);
  ...
  return (
    <div style={mock.scrollPad}>
      <div style={mock.eyebrow}>FILE 07 · BINDER</div>
      ...
      <div style={{ fontSize: 10.5, color: 'var(--ink-3)', marginBottom: 14 }}>
        148 pages · ready in 2 minutes
      </div>
      ...
      {/* TOC items checking off */}
      <div style={{ fontFamily: 'var(--mono)', fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.14em', color: 'var(--ink-3)', marginBottom: 8 }}>CONTENTS</div>
      {[
        ...
        return (
          <div key={i} style={{
            ...
            <span className="num" style={{ fontSize: 9, color: 'var(--ink-3)' }}>{r.p === progress ? '...' : ''}</span>
          </div>
        );
      })}
      ...
      {progress >= 100 && (
        <div className="ifade" style={{
          ...
          <div style={{ fontFamily: 'var(--serif)', fontSize: 14, fontStyle: 'italic', marginBottom: 4 }}>Binder ready.</div>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.14em', opacity: 0.7 }}>Tap to download · 8.4 MB</div>
        </div>
      )}
    </div>
  );
...
const FloatingAnnotations = ({ active }) => {
      ...
      {items.map((a, i) => (
        <div
          ...
          style={{
            ...
            animation: `annotIn 720ms cubic-bezier(0.2,0.7,0.2,1) ${300 + i * 220}ms forwards, annotFloat 5s ease-in-out ${1500 + i * 220}ms infinite`,
            pointerEvents: 'none',
          }}
        >
...
const AnnotCard = ({ side, title, sub, icon }) => {
  ...
  return (
    <div style={{
      ...
      {/* Connector line pointing toward phone */}
      <div style={{
        position: 'absolute',
...
// ─── Mock styles + global animations injected ───────────────────────────────
const mock = {
  scrollPad: { padding: '14px 18px 20px', height: '100%', overflow: 'hidden' },
  eyebrow: { fontFamily: 'var(--mono)', fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.16em', color: 'var(--ink-3)' },
};
const mockMonogram = {
  width: 18, height: 18, borderRadius: 99,
  border: '1px solid var(--ink-2)',
  display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
  fontFamily: 'var(--serif)', fontSize: 9, color: 'var(--ink)',
  background: 'var(--paper)',
};

// Inject keyframes once
(function injectKeyframes() {
  if (document.getElementById('mockup-anim-css')) return;
  const s = document.createElement('style');
  s.id = 'mockup-anim-css';
  s.textContent = `
    @keyframes scanY {
    ...
    @keyframes annotIn {
      0%   { opacity: 0; transform: translateX(var(--ax, 30px)) translateY(8px); }
    ...
    @media (max-width: 980px) {
      .mockup-wrap [class] { /* ensure mobile annotations hide cleanly */ }
    }
  `;
