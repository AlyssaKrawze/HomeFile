...
interface HomeContactsImportModalProps {
  homeId: string
  onClose: () => void
}
...
export default function HomeContactsImportModal({ homeId, onClose }: HomeContactsImportModalProps) {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const res = await fetch(`/api/homes/${homeId}/contacts/import-map`, {
        method: 'POST',
  ...
  async function handleSave() {
    ...
    const { error: dbError } = await supabase.from('home_contacts').insert(
      toSave.map(c => ({
        home_id: homeId,
        name: c.name.trim(),
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">

        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E0D9D0] flex-shrink-0">
          <div>
            <h2 className="font-semibold text-[#2F3437]">Import Vendors &amp; Contacts</h2>
            <p className="text-xs text-slate-500 mt-0.5">
              {step === 'upload' && 'Upload an Excel or CSV file to bulk-add contacts to your home directory'}
              ...
              {step === 'done' && `${savedCount} contact${savedCount !== 1 ? 's' : ''} added to your home directory`}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
            ...
            <div>
              ...
              >
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">Works with any contact export — Google Contacts, phone backups, custom lists</p>
                </div>
              </button>
                  ...
                  <tbody className="divide-y divide-[#F0EBE3]">
                    {contacts.map((c, i) => (
                      <tr key={i} className={!c._include ? 'opacity-40' : ''}>
                        ...
                        <td className="px-2 py-1"><input type="text" value={c.name} onChange={e => updateContact(i, 'name', e.target.value)} className={`${inputCls} min-w-[100px]`} /></td>
                        <td className="px-2 py-1"><input type="text" value={c.company} onChange={e => updateContact(i, 'company', e.target.value)} className={`${inputCls} min-w-[100px]`} /></td>
                        <td className="px-2 py-1"><input type="text" value={c.category} onChange={e => updateContact(i, 'category', e.target.value)} className={`${inputCls} min-w-[90px]`} /></td>
                        <td className="px-2 py-1"><input type="text" value={c.phone} onChange={e => updateContact(i, 'phone', e.target.value)} className={`${inputCls} min-w-[90px]`} /></td>
                        <td className="px-2 py-1"><input type="text" value={c.email} onChange={e => updateContact(i, 'email', e.target.value)} className={`${inputCls} min-w-[130px]`} /></td>
                        <td className="px-2 py-1"><input type="text" value={c.notes} onChange={e => updateContact(i, 'notes', e.target.value)} className={`${inputCls} min-w-[100px]`} /></td>
                        <td className="px-3 py-1.5">
          ...
          {step === 'done' && (
            <div className="flex flex-col items-center gap-4 py-12">
              ...
              <div className="text-center">
                <p className="text-sm font-semibold text-[#2F3437]">{savedCount} contact{savedCount !== 1 ? 's' : ''} added</p>
                <p className="text-xs text-slate-400 mt-1">Added to your home&apos;s vendor directory.</p>
              </div>
              <button
                onClick={onClose}
        ...
        {step === 'preview' && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-[#E0D9D0] flex-shrink-0 bg-white">
            ...
            >
              Add {includedCount} contact{includedCount !== 1 ? 's' : ''} to directory
            </button>
          </div>
        )}
      </div>
