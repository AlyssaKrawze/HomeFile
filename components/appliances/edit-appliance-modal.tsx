...
// Sub-component: fresh useState on every mount (guarantees category pre-population)
function EditApplianceForm({ appliance, onClose }: { appliance: Appliance; onClose: () => void }) {
...
export default function EditApplianceModal({ appliance }: EditApplianceModalProps) {
  ...
  return (
    <>
      ...
      {open && (
        <EditApplianceForm appliance={appliance} onClose={() => setOpen(false)} />
      )}
    </>
  )
}
